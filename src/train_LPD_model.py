...
class ALPR(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.block1 = nn.Sequential(
            nn.Conv2d(3, 16, 5, padding=2), # (3, 384, 384) -> (16, 128, 128)
            InceptionResidual(16, 16, 5, 3, 2, include_max_pool=True), #(16, 128, 128)
        )
        self.block2 = nn.Sequential(
            nn.Conv2d(16, 24, 3, padding=1),  # (16, 128, 128) -> (24, 64, 64)
            InceptionResidual(24, 24, 5, 3, 3, include_max_pool=True), # (24, 64, 64)
        )
        self.block3 = nn.Sequential(
            nn.Conv2d(24, 32, 3, padding=1),  # (24, 128, 128) -> (32, 32, 32)
            InceptionResidual(32, 32, 5, 3, 3, include_max_pool=True) , # (32, 32, 32) -> (32, 32, 32)
        )
        self.block4 = nn.Sequential(
            nn.Conv2d(32, 32, 3, padding=1),  # (32, 32, 32) -> (32, 16, 16)
            InceptionResidual(32, 32, 5, 3, 3, include_max_pool=True),  # (32, 16, 16) -> (32, 16, 16)
        )
        self.block5 = nn.Sequential(
            nn.Conv2d(32, 48, 3, padding=1),  # (32, 16, 16) -> (48, 8, 8)
        ...
        self.anchors = AnchorGenerator(sizes=((39,), (79,)), aspect_ratios=((2.0,), (2.0,))
    )
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, Any]:
        ...
        anchors = self.anchors(images, [fm0, fm1])
